...
export class TrainerCustomerList extends Component {
     ...
     this.state ={   
      ...
      db.transaction((txn)=> {
        ...
        for (let i = 0; i < res.rows.length; ++i) {
         ...
         console.log('temp:',"SELECT * FROM customer_reg where trainerId='"+temp[0].trainer_id+"'");

txn.executeSql("SELECT * FROM customer_reg where trainerId='"+temp[0].trainer_id+"'",[],(tx, res)=>{
          console.log('item:', res.rows.length);
          var temp = [];
          if (res.rows.length != 0) {
        for (let i = 0; i < res.rows.length; ++i) {
...
export default TrainerCustomerList
